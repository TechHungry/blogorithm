// src/app/profile/edit/[slug]/page.tsx
'use client';
...
export default function EditPostPage() {
    ...
    const fetchPost = async () => {
        try {
            const response = await fetch(`/api/posts/get-by-slug?slug=${slug}`);
            ...
            if (response.ok) {
                ...
                setFormData({
                    ...
                    tags: data.post.tags || [],
                    tagInput: '',
                    content: data.post.bodyHtml || '',
                });

                // Set flag to indicate editor should be initialized with content
                setEditorInitialized(true);

                // Set preview URL for cover image
                if (data.post.coverImage) {
                    // Create a Sanity image URL
                    const imageRef = data.post.coverImage.asset._ref;
                    // Convert the reference to a URL format
                    const imageUrl = `https://cdn.sanity.io/images/${data.projectId}/${data.dataset}/${imageRef
                        .replace('image-', '')
                        .replace('-jpg', '.jpg')
                        .replace('-png', '.png')
                        .replace('-webp', '.webp')
                ...
                alert('Failed to load post. Redirecting to profile.');
                router.push('/profile');
            }
        } catch (error) {
    ...
    // Handle editor content changes - use functional state update to ensure we don't lose state
    const handleEditorChange = useCallback((htmlContent: string) => {
        setFormData(prevState => ({...prevState, content: htmlContent}));
    }, []);

    // Tag input handlers
    const handleTagInputChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
        setFormData(prevState => ({...prevState, tagInput: e.target.value}));
    }, []);
    ...
    const handleTagKeyDown = useCallback((e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Enter' && formData.tagInput.trim()) {
            ...
            setFormData(prevState => ({
                ...prevState,
                tags: [...prevState.tags, prevState.tagInput.trim()],
                tagInput: ''
            }));
    ...
    }, [formData.tagInput, formData.tags.length]);
    ...
    const handleSubmit = useCallback(async (e: React.FormEvent, isDraft: boolean = false) => {
        ...
        try {
            ...
            // Add the cover image if a new one was selected
            if (imageFile) {
    ...
    }, [formData, imageFile, post, router]);
                ...
                <form onSubmit={(e) => handleSubmit(e, false)} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div
                                className="flex flex-wrap items-center bg-gray-700 text-white rounded p-2 border border-gray-600 focus-within:border-blue-500 min-h-[42px]">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div
                                className="flex flex-col h-32 border border-gray-600 bg-gray-700 rounded overflow-hidden">
                                <div
                                ...
                                >
                                    {previewUrl ? (
                                        <span
                                            className="truncate max-w-[150px]">{imageFile?.name || 'Current image'}</span>
                                    ) : (
                                    ...
                                    <div className="flex space-x-2">
                                        ...
                                        {previewUrl && (
                                            <button
                                                type="button"
                                                onClick={() => {
                                                    if (post.coverImage && !imageFile) {
                                                        // If using original image, keep the preview URL
                                                        return;
                                                    }
                                                    setImageFile(null);
                                                    setPreviewUrl('');
                                ...
                                {previewUrl ? (
                                    <div
                                        className="flex-1 bg-[#161515] flex items-center justify-center p-2 overflow-hidden">
                                        <img
                                            src={previewUrl}
                                            alt="Cover preview"
                    ...
                    {/* Content Editor - Only render when data is loaded */}
                    <div>
